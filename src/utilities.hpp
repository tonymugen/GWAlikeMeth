...
void bracketMax(T &func, const double &startA, const double &startB, double &candA, double &candB, double &candC){
	...
	if (fabs(fa - fb) < 1.001*EPS) {
		candB += 100.0; // maybe just unlucky; change candB
		fb = func(candB);
		if (fabs(fa - fb) < 1.001*EPS) { // now for sure there is a problem
...
template<class T>
void maximizer(T &func, const double &startX, double &xMax, double &fMax){
	const double tol         = 1.001 * sqrt(EPS); // set the tolerance just above the theoretical limit
	...
	const double ZEPS        = 1e-3 * EPS;        // small number to protect against numerical problems when the maximum is zero and we are trying to achieve a certain fractional accuracy
...
double betacf(const double &x, const double &a, const double &b){
	...
	if (fabs(denom) < FPMIN) denom = FPMIN; // set d to something resonalbly small if it is too close to 0
	denom = 1.0/denom;
	...
	for (unsigned int m = 1; m < maxIter; m++) {
		...
		denom = 1.0 + aa*denom;
		numer = 1.0 + aa/numer;
		if (fabs(denom) < FPMIN) denom = FPMIN;
		if (fabs(numer) < FPMIN) numer = FPMIN;
		...
		cFrac *= del;
		if (fabs(del-1.0) <= EPS) break; // done if within epsilon
